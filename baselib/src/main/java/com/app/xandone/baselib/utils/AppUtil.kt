...
object AppUtil {
     ...
     */
    val appName: String
        get() = sApp.resources.getString(sApp.applicationInfo.labelRes)
...
//            val appChannel = getApplicationMetaData("APP_CHANNEL")
            ...
            var uuid = SpHelper.getDefaultString(sApp, "uuid")
            if (uuid!!.isNotEmpty()) {
                deviceSerial = uuid
                return deviceSerial.toString()
            }
            uuid = UUID.randomUUID().toString().replace("-", "").uppercase(Locale.getDefault())
            SpHelper.save2DefaultSp(sApp, "uuid", uuid)
            deviceSerial = uuid
            return deviceSerial.toString()
        } else {
            return deviceSerial.toString()
    ...
    fun getString(resId: Int): String {
        return sApp.resources.getString(resId)
    }
    ...
    fun getDimension(resId: Int): Int {
        return sApp.resources.getDimensionPixelOffset(resId)
    }
    ...
    fun getResourceId(name: String, type: String): Int {
        return sApp.resources.getIdentifier(name, type, appPackage)
    }
    ...
    fun isInstalled(packageName: String): Boolean {
        val packageInfo: PackageInfo? = try {
            sApp.packageManager.getPackageInfo(packageName, 0)
        } catch (e: PackageManager.NameNotFoundException) {
            null
        }
    ...
    fun getAppIcon(): Drawable {
        val packageManager = sApp.packageManager
